...
.知識產權MetroCluster
link:https://www.netapp.com/pdf.html?item=/media/13481-tr4689.pdf["TR-4689 ： NetApp MetroCluster IP"^] MetroCluster IP 是適用於運作在 FAS 和 AFF 系統上的 ONTAP 的持續可用儲存解決方案。MetroCluster IP 是最新一代的儲存方案，採用基於乙太網路的後端儲存網路。MetroCluster IP 提供高度備援的組態，以滿足最關鍵業務應用的需求。MetroCluster IP 已整合到 ONTAP 中，並為使用 ONTAP 儲存設備的用戶端和伺服器提供 NAS 和 SAN 連線。
